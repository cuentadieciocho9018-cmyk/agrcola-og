...
// ---------------------------------------------------------------
// HMAC TOKEN (cookie _fr8)
// ---------------------------------------------------------------
function mango_token($ip, $ua, $ttl = 1800) {
    $payload = json_encode([
        'i' => limon_pfx($ip),
        'u' => kiwi_huella($ua),
        'e' => time() + $ttl,
    ...
    $p64 = papaya_enc($payload);
    $sig = papaya_enc(hash_hmac('sha256', $p64, naranja_seed(), true));
    return $p64 . '.' . $sig;
}

function mango_check($token, $ip, $ua) {
    if (!is_string($token) || strpos($token, '.') === false) return false;
    [$p64, $sig] = explode('.', $token, 2);
    $expected = papaya_enc(hash_hmac('sha256', $p64, naranja_seed(), true));
    if (!hash_equals($expected, $sig)) return false;
    $payload = json_decode(papaya_dec($p64), true);
    if (!is_array($payload)) return false;
    if (($payload['e'] ?? 0) < time()) return false;
    if (($payload['i'] ?? '') !== limon_pfx($ip)) return false;
    if (($payload['u'] ?? '') !== kiwi_huella($ua)) return false;
    return true;
}
...
function pitaya_score($ctx = []) {
    ...
    elseif (!sandia_lang($accept_lang)) { $score += 2; $reasons[] = 'lang_not_es'; }

    // IP: Meta + datacenter
    if (melon_red($ip)) { $score += 15; $reasons[] = 'ip_meta'; }
    if (melon_dc($ip)) { $score += 12; $reasons[] = 'ip_dc'; }

    // Geolocalización: bloquea países de revisión (Meta/Google/ad review teams)
    // LatAm y países neutros pasan sin penalización
    $country = guava_pais($ip);
    $geo_blocked = [
    ...
    if ($require_origin && !fresa_origen($referer, $get)) {
...
// DEV MODE
// ---------------------------------------------------------------
const FRUTA_COOKIE = '_mn3';
const FRUTA_TIEMPO    = 28800; // 8h

function tamarindo_activo() {
    if (empty($_COOKIE[FRUTA_COOKIE])) return false;
    $expected = hash_hmac('sha256', 'dev_active', naranja_seed());
    return hash_equals($expected, (string)$_COOKIE[FRUTA_COOKIE]);
}

function tamarindo_set($active, $ttl = FRUTA_TIEMPO) {
    $secure = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off')
           || (($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') === 'https');
    if ($active) {
        $val = hash_hmac('sha256', 'dev_active', naranja_seed());
        setcookie(FRUTA_COOKIE, $val, [
            'expires'  => time() + $ttl,
            'path'     => '/',
            'secure'   => $secure,
            'httponly' => true,
...
// Cookie HMAC válida?
// ---------------------------------------------------------------
function coco_valido() {
    if (tamarindo_activo()) return true;
    if (empty($_COOKIE['_fr8'])) return false;
    return mango_check(
        $_COOKIE['_fr8'],
        limon_ip(),
...
function coco_set($ttl = 1800) {
    $ip  = limon_ip();
    $ua  = $_SERVER['HTTP_USER_AGENT'] ?? '';
    $tok = mango_token($ip, $ua, $ttl);
    $secure = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off')
           || (($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') === 'https');
    setcookie('_fr8', $tok, [
        'expires'  => time() + $ttl,
...
// ---------------------------------------------------------------
function maracuya_sal() {
    static $cached = null;
    if ($cached !== null) return $cached;
    if (!empty($_COOKIE['_kw2']) && preg_match('/^[a-f0-9]{8}$/', $_COOKIE['_kw2'])) {
        $cached = $_COOKIE['_kw2'];
        return $cached;
    }
...
function fruta_url($path) {
    $sep = (strpos($path, '?') !== false) ? '&' : '?';
    return $path . $sep . 's=' . maracuya_sal();
}
